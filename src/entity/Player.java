	...
	public void deplacement(int code){
		...
		if(code == 37 || code == 81){
			...
			for(int i=0; i<items.size();i++){
				if(items.get(i).itemName == "sword"){
					b = true;
				}
				if(b){
					try {
						m_idleImage = ImageIO.read(getClass().getResource("/Player/STEVE_EPEE_g.png"));								
					} catch (IOException e) {
						e.printStackTrace();
					}					
				}
				...
				if(entities.get(i).name == "chest"){
					...
					if (etilex == tilex && etiley == tiley-1) {
						...
						if (((Chest) c).getDispo()){
							...
							try {
								(entities.get(i)).m_idleImage = ImageIO.read(getClass().getResource("/tiles/sandstone_top.png"));
								entities.remove(i);
							} catch (IOException e) {
								e.printStackTrace();
							}
